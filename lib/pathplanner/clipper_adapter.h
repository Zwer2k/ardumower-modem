...
inline std::vector<Polygon> clipDifferenceOpen(const std::vector<Polygon> &subjects,
    const std::vector<Polygon> &clips) {
    Clipper64 c;
    c.AddOpenSubject(toI64(subjects));
    c.AddClip(toI64(clips));
    Paths64 closed, open;
    c.Execute(ClipType::Difference, FillRule::NonZero, closed, open);
    auto result = toDbl(open);
    return result;
}

inline std::vector<Polygon> clipDifferenceOpen(const std::vector<Polygon> &subjects,
    const Polygon &clip) {
    return clipDifferenceOpen(subjects, std::vector<Polygon>{clip});
}

// Return only polygons with positive signed area (outer boundaries), discarding
// holes that Clipper2 returns as separate paths in a Difference result.
inline std::vector<Polygon> keepOuterPolygons(const std::vector<Polygon> &polys) {
    std::vector<Polygon> result;
    for (const auto &p : polys) {
        if (polygonArea(p) > 0.0) result.push_back(p);
    }
    return result;
}

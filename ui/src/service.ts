  ...
  public async load(): Promise<Settings> {
    ...
    try {
      ...
      throw new ServiceError(
        "load settings",
        { action: "decode", error: err.message },
        0,
        "",
      );
    }
  ...
  public async save(s: Settings, original: Settings): Promise<Settings> {
    try {
      ...
      if (err instanceof ServiceError) {
        if (err.details.action === "validate") Invalid.set(err.details.error);
      }
      throw err;
    }
  }
...
const keyAt = (o: any, path: string[]) =>
  path.length === 0 ? o : keyAt(o[path[0]], path.slice(1));

const strip = (a: any, b: any, path: string[]): boolean =>
  keyAt(a, path) !== keyAt(b, path);
...
class RobotCommandServiceClass {
  public async send(
    action: RobotAction,
    payload?: Record<string, any>,
  ): Promise<RobotCommandResult> {
    ...
    await assertOk(res, "robot command");
    return res.json();
  }
}

export const RobotCommandService = new RobotCommandServiceClass();
